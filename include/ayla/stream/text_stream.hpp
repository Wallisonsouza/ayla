...
public:
  struct State {
    ...
    ayla::source::SourceSpan span_to(const State &end) const { return ayla::source::SourceSpan{ptr, end.ptr}; }
...
public:
  explicit TextStream(const ayla::source::SourceBuffer &buf) : buffer(buf), begin(buf.begin()), current(buf.begin()), end(buf.end()) {}
