...
core::ast::ASTStatementNode *Parser::parse_import_statement() {
  ...
  if (!unit.tokens.match(ayla::structural::token::TokenKind::IMPORT_KEYWORD)) return nullptr;

  std::vector<core::ast::IdentifierNode *> path_nodes;

  // primeiro identificador é obrigatório
  auto *name_token = unit.tokens.match(ayla::structural::token::TokenKind::IDENTIFIER);
  if (!name_token) {
  ...
  while (true) {
    ...
    if (!unit.tokens.match(ayla::structural::token::TokenKind::DOT)) break;

    name_token = unit.tokens.match(ayla::structural::token::TokenKind::IDENTIFIER);
    if (!name_token) {
