...
ayla::ast::TypeNode *TypeParser::parse_type() {
  auto *name = parser.names().parse_name();

  if (!name) return nullptr;

  std::vector<ayla::ast::TypeNode *> generic_arguments;

  // futuramente:
  // if (tokens.match(TokenKind::LESS))
  //     generic_arguments = parse_generic_arguments();

  return context.ast().create_node<ayla::ast::TypeNode>(name, std::move(generic_arguments));
}