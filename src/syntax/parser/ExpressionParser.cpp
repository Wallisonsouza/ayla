...
ayla::ast::ExpressionNode *ExpressionParser::parse_binary_expression(int min_bp, ayla::ast::ExpressionNode *left) {
  while (true) {
    ...
    auto *info = context.operators().get(token->descriptor->kind);

    if (!info || info->kind != core::table::OperatorKind::Infix) break;
    ...
    left = context.ast().create_node<ayla::ast::node::BinaryExpressionNode>(left, op, right);
  }
...
ayla::ast::ExpressionNode *ExpressionParser::parse_postfix_expression() {
  ...
  while (true) {
    ...
    if (!info || info->kind != core::table::OperatorKind::Postfix) break;
...
ayla::ast::ExpressionNode *ExpressionParser::parse_unary_expression() {
  ...
  if (!info || info->kind != core::table::OperatorKind::Prefix) { return parse_postfix_expression(); }
