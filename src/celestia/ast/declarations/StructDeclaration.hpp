...
struct StructDeclaration : NamedDeclaration {
  ...
  StructDeclaration(IdentifierNode *name, std::vector<TypeNode *> compositions, std::vector<FieldDeclaration *> fields, DeclarationSpecifiers specifiers)
      : NamedDeclaration(NodeKind::StructDeclaration, name, specifiers), compositions(std::move(compositions)), fields(std::move(fields)) {}
};
} // namespace celestia::ast
