...
public:
  void run(CompilerEnvironment &env, CompilationUnit &unit) override {
    ...
    if (unit.module.is_valid()) return;

    auto name = unit.ast_module->name->get_name();

    unit.module = env.modules.register_module(name);

    if (!unit.module.is_valid()) throw std::runtime_error("failed to create module: " + name);
  }
};
...
public:
  void run(CompilerEnvironment &env, CompilationUnit &unit) override {
    ...
    if (!unit.module.is_valid()) throw std::runtime_error("builtin module not initialized");
  }
};