...
void TypeChecker::check(ayla::ast::AstNode *node) {
  ...
  switch (node->kind) {
  case ayla::ast::NodeKind::ExpressionStatement: check_expression_statement(static_cast<ayla::ast::node::ExpressionStatementNode *>(node)); break;
  case ayla::ast::NodeKind::Identifier: check_identifier(static_cast<ayla::ast::node::IdentifierExpressionNode *>(node)); break;
  ...
  case ayla::ast::NodeKind::VariableDeclaration: check_variable_declaration(static_cast<ayla::ast::node::VariableDeclarationNode *>(node)); break;
  ...
  case ayla::ast::NodeKind::MemberAccess: check_member_access(static_cast<ayla::ast::node::MemberAccessExpressionNode *>(node)); break;
  ...
  case ayla::ast::NodeKind::WhileStatement: check_while_statement(static_cast<ayla::ast::node::WhileStatementNode *>(node)); break;
...
void TypeChecker::check_identifier(ayla::ast::node::IdentifierExpressionNode *node) {
  if (!node) return;
...
void TypeChecker::check_expression_statement(ayla::ast::node::ExpressionStatementNode *node) {
...
void TypeChecker::check_object_literal(ayla::ast::node::ObjectLiteralNode *node) {
  ...
  for (auto *field : node->fields) {
    ...
    auto *key = static_cast<ayla::ast::node::IdentifierExpressionNode *>(field->key);
...
void TypeChecker::check_variable_declaration(ayla::ast::node::VariableDeclarationNode *node) {
  if (!node) return;
  ...
  auto *idPattern = dynamic_cast<ayla::ast::IdentifierPatternNode *>(node->pattern);
  if (!idPattern) return;
...
void TypeChecker::check_pattern(ayla::ast::PatternNode *node) {
  if (!node) return;

  if (auto *idPattern = dynamic_cast<ayla::ast::IdentifierPatternNode *>(node)) {
...
void TypeChecker::check_while_statement(ayla::ast::node::WhileStatementNode *node) {
  check(node->condition);
  if (node->condition->inferred_type != &BuiltinTypes::Boolean) report_error(DiagnosticCode::TypeMismatch, node->condition->slice);
...
void TypeChecker::check_member_access(ayla::ast::node::MemberAccessExpressionNode *node) {
  check(node->base);
  Type *baseType = node->base->inferred_type;
...
void TypeChecker::check_assignment(ayla::ast::node::AssignmentExpressionNode *node) {
  ...
  case ayla::ast::NodeKind::Identifier: {
    auto *idNode = static_cast<ayla::ast::node::IdentifierExpressionNode *>(node->target);
    targetType = idNode->inferred_type ? idNode->inferred_type : &BuiltinTypes::Unknown;
    break;
  }

  case ayla::ast::NodeKind::MemberAccess: {
    auto *memNode = static_cast<ayla::ast::node::MemberAccessExpressionNode *>(node->target);
